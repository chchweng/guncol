      ...
      <main className="w-full max-w-5xl bg-gray-800 shadow-2xl rounded-lg p-4 md:p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            {/* Download Button - Appears after an image is successfully uploaded */} 
            {uploadedImageUrl && !isLoading && !isSegmenting && !isRecoloring && (
              <div className="mt-6">
                <button 
                  onClick={() => {
                    if (!uploadedImageUrl || !uploadedImageName) return;
                    const link = document.createElement('a');
                    link.href = uploadedImageUrl.split('?')[0]; // Use base URL without cache-buster
                    link.download = uploadedImageName; // Use the current image name for download
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
                  }}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300 ease-in-out flex items-center justify-center space-x-2"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
